...
export const useDraggable = ({ defaultX = 0, defaultY = 0 } = {}) => {
  ...
  const [position, setPosition] = useStateRef({ x: defaultX, y: defaultY })
  ...
  const onMouseMove = React.useCallback(
    (e) => {
      ...
      if (resizeDirectionRef.current === 'horizontal') {
        setSize({ width: clientX - left, height })
      } else if (resizeDirectionRef.current === 'vertical') {
        setSize({ width, height: clientY - top })
      } else if (resizeDirectionRef.current === 'both') {
        setSize({ width: clientX - left, height: clientY - top })
      }
    },
    [isResizingRef, resizeDirectionRef, setSize]
  )
