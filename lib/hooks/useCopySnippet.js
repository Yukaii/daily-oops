...
const isCopyButton = button => button && button.classList.contains('copy-snippet-button') && !button.classList.contains('copied')
...
export const useCopySnippet = () => {
  useEffect(() => {
    const onClick = function (e) {
      ...
      const code = [...pre.querySelectorAll('.hljs-line')].map(l => l.textContent).join
      ('\n')
      copy(code)
