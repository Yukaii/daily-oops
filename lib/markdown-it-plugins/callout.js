...
function calloutRule(state, startLine, endLine, silent) {
  ...
  if (
    ...
    if (state.src.charCodeAt(pos) === 0x5d /* ] */) {
      ...
      // We have a callout
      if (!silent) {
        var token = state.push('callout_open', 'callout', 1)
        token.markup = '> [!'
...
function renderCalloutOpen(tokens, idx, options, env, self) {
  var token = tokens[idx]

  const className = calloutToClassMap[token.info] || 'info'
  const icon = calloutToIconMap[token.info] || 'info-circle'

  return (
    `<div class="callout callout-${className}" data-label="${token.info}">` +
    `<span class="callout-icon"><i class="fas fa-fw fa-${icon}"></i></span>`
  )
}

function renderCalloutClose(tokens, idx, options, env, self) {
  return '</div>'
}
